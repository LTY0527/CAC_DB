...
function buildOption(topList = []) {
  return {
    ...
    yAxis: { type: 'category', data: topList.map((item) => String(item.top_potential_student_id)), axisLabel: axisLabelStyle, axisLine: axisLineStyle },
    series: [
      {
        name: '匹配分',
        type: 'bar',
...
export default function EnrollmentMatching({ enrollmentData = [], enrollmentEvalData = [], dataLoadedAt = '', loading, error }) {
  ...
  const columns = [
    ...
    { title: '推荐原因', render: (_, record) => `该画像下历史样本在 ${record.target_major} 的平均匹配分较高。` },
  ]
  ...
  return (
    <Row gutter={[16, 16]}>
      <Col span={24}>
        <Card style={panelStyle}>
          <Row gutter={[16, 16]} align="middle">
            <Col xs={24} xl={16}><div style={sectionTitleStyle}>招生匹配（协同过滤）</div></Col>
            <Col xs={24} xl={8}>
      ...
      <Col xs={24} md={12} xl={6}><Card style={panelStyle}><Statistic title="当前专业" value={currentMajor || '-'} styles={{ title: statTitleStyle, content: statValuePrimary }} /></Card></Col>
      <Col xs={24} md={12} xl={6}><Card style={panelStyle}><Statistic title="平均匹配分" value={avgScore} precision={2} styles={{ title: statTitleStyle, content: statValueBlue }} /></Card></Col>
      <Col xs={24} md={12} xl={6}><Card style={panelStyle}><Statistic title="平均样本量" value={avgSample} precision={0} styles={{ title: statTitleStyle, content: statValueCyan }} /></Card></Col>
      <Col xs={24} md={12} xl={6}><Card style={panelStyle}><Statistic title="Precision@K" value={Number(evalMap['Precision@K']?.metric_value || 0)} precision={3} styles={{ title: statTitleStyle, content: statValuePurple }} /></Card></Col>

      <Col span={24}>
        <Card title={<span style={sectionTitleStyle}>筛选条件</span>} style={panelStyle}>
          <Row gutter={[12, 12]}>
            <Col><Select value={currentMajor || undefined} onChange={setMajor} options={majors.map((item) => ({ label: item, value: item }))} style={{ width: 220 }} placeholder="选择专业" /></Col>
            <Col><Select value={topN} onChange={setTopN} style={{ width: 140 }} options={[{ label: 'Top 5', value: 5 }, { label: 'Top 10', value: 10 }, { label: 'Top 15', value: 15 }]} /></Col>
            <Col><Select value={minScore} onChange={setMinScore} style={{ width: 180 }} options={[{ label: '全部候选', value: 0 }, { label: '优先关注', value: 7.5 }, { label: '重点跟进', value: 7.8 }, { label: '核心目标', value: 8.0 }]} /></Col>
          </Row>
        </Card>
      </Col>
